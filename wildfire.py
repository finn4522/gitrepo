...
def plot_points(
    fire_map: graphics.utm_plot.Map,
    ...
    for point in points:
        ...
        symbol = fire_map.plot_point(easting, northing, size_px=size_px, color=color)
...
def centroid(points: list[tuple[int, int]]) -> tuple[int, int]:
    ...
    >>> centroid(get_fires_utm("data/test_locations_utm.csv"))
    [678096, 4900384]
    ...
    for point in points:
        sumx += point[0]
        sumy += point[1]
    meanxy = (sumx // len(points), sumy // len(points))
    return meanxy
...
def closest_index(point: tuple[int, int], centroids: list[tuple[int, int]]) -> int:
    ...
    for centroid in centroids:
        distances.append(sq_dist(point, centroid))
    closest_centroid = distances.index(min(distances))
    return closest_centroid


def assign_closest(
    points: list[tuple[int, int]], centroids: list[tuple[int, int]]
) -> list[list[int, int]]:
...
def move_points(
    fire_map: graphics.utm_plot.Map, points: list[tuple[int, int]], symbols: list
):
